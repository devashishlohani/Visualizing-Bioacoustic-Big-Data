...
class Dataset:
    ...
    def __init__(self, data,labels, normalize='y'): # normalize is set by default to 'y' or yes, meaning that it will automatically normalize the data.

        self.__data = data   # To store the data
    ...
    @classmethod
    def import_Timit_Data(cls,path,f=0):
        ...
        labels = []  # To store the labels of the corresponding data
        speakers = []  # To store the speakers corresponding to the data
        dialects = []  # To store the dialects corresponding to the data


        dataset = joblib.load(path)  # joblib.load for loading the file(which contains a dictionary of filenames as keys and datavalues as values) to the dataset variable
        ...
        if (f == 0):
            for (k, v) in dataset.items():
                ...
                data += [i[1] for i in v]  # Get the list of the chirplet data for the current audio and append it to the __data array
                label = [i[0] for i in v]  # Get the list of labels of the data for the current audio
                labels += label  # Append the label list to the __labels array
                speakers += [s] * len(label)  # Add the same speaker in the speakers array for the current audio(since any single audio is only spoken by one speaker)
                dialects += [d] * len(label)  # Add the same speaker in the speakers array for the current audio
        else:
            for (k, v) in dataset.items():
                d = k.split("/")[1]  # Get dialect of the current audio from the key
                s = k.split("/")[2]  # Get speaker of the current audio from the key
                data += [i[1][0] for i in v]  # Get the list of the chirplet data for the current audio and append it to the __data array
                label = [i[0] for i in v]  # Get the list of labels of the data for the current audio
                labels += label  # Append the label list to the __labels array
                speakers += [s] * len(label)  # Add the same speaker in the speakers array for the current audio(since any single audio is only spoken by one speaker)
                dialects += [d] * len(label)  # Add the same speaker in the speakers array for the current audio
